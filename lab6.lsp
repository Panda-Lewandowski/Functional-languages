...
;(reduce #'+ ()) -> 0
;(reduce #'* ()) -> 1
;	Сначала функция проверяет список-аргумент. Если он пуст, возвращается значение 
; функции при отсутствии аргументов.
; Также reduce использует аргумент :initial-value. Этот аргумент определяет значение, 
; к которому будет применена функция при обработке первого элемента списка-аргумента. 
; Если список-аргумент пуст, то будет возвращено значение initial-value.
